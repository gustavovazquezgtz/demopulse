...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const [people, teams, demos] = await Promise.all([
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {people.length === 0 && teams.length === 0 && demos.length === 0 && (
      ...
      {teams.length > 0 && (
        <Card>
          <CardHeader><CardTitle>Teams / Projects</CardTitle></CardHeader>
          <CardContent className="flex flex-col divide-y divide-border">
            {teams.map((t) => (
            ...
            {demos.map((d) => (
              <Link key={d.id} href={`/demos/${d.id}`} className="flex items-center justify-between py-2.5">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{d.teams.map((t) => t.team.name).join(", ") || "—"}</p>
                </div>
